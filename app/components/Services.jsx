...
const Services = ({isDarkMode, setIsDarkMode}) => {
  ...
  className={`${montserrat.className} text-center mb-2`}>What I offer</h4>
      <h1
      // initial={{y:-20, opacity:0}}
      // whileInView={{y:0,opacity:1}}
      // transition={{duration:1, delay:0.5}}
      className={`${poppins.className} text-2xl text-center font-semibold`}>My Services</h1>
      ...
      className={`${poppins.className} flex flex-col  gap-6 mt-8`}>
        <div
        // initial={{scale:0.7, opacity:0}}
        // whileInView={{scale:1,opacity:1}}
        // transition={{duration:1, delay:1.1}}
        className='border-[0.5px] border-gray-400  rounded-xl p-6 cursor-pointer  hover:shadow-lg hover:bg-green-100 hover:-translate-y-1 duration-500 dark:bg-stone-700 dark:hover:bg-lime-700'>
